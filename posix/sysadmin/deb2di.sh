...
(
	...
	cat <<-'EOS'
		...
		set +e
		yn=
		case $(dpkg-query -Wf '${Status}\n' console-setup 2>/dev/null) in
		(*\ ok\ not-installed|)
			case $(dpkg-query -Wf '${Status}\n' console-data 2>/dev/null) in
			(*\ ok\ not-installed|)
				yn=--defaultno ;;
			esac ;;
		esac
